function [C1, varargout] = map(f, varargin)
%fun.map Like cellfun and arrayfun, but always returns a cell array
%
%   [c1, ...] = fun.map(f, a1, ...) applies the function `f` to each
%   element of the variable number of arrays `a1, a2,...` passed in. The
%   outputs of `f` are used to build up cell arrays for each output.
%
%   Unlike MATLAB's cellfun and arrayfun, fun.map(..) can take a mixture
